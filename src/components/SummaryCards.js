...
const SummaryCards = () => {
  ...
  const kesBalance = kesAccount ? kesAccount.balance : 0;
    ...
    <div className="bg-white p-4 space-y-4 md:grid md:grid-cols-4 md:gap-4 md:space-y-0">
     ...
     <div className="bg-white p-4 rounded-lg border border-gray-200 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-2 md:text-2xl">KSh {kesBalance}</p> {/* Display KES balance */}
        </div>
        <div className="rounded-full bg-yellow-500 bg-opacity-20 p-2"> {/* Chose yellow for differentiation */}
          <AiFillAlipayCircle className="text-yellow-500 text-xl md:text-3xl" /> {/* Generic money icon */}
        </div>
      </div>
